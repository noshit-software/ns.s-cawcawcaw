...
Hit **SAVE**. Projects can be renamed or deleted from the expanded view. Renaming updates all references across queue, post history, commit buffer, and activity log. Project names are normalized to uppercase everywhere — no case-mismatch duplicates.
...
**MARK PUBLISHED** sets a post's status to published without sending anything. On published posts, the REPUBLISH TO dropdown includes per-platform "MARK X ✓" options that record a platform as already published without re-sending — for posts that went out externally.
...
The writer skips stories about internal plumbing, config changes, and incremental fixes — only posts about problems people recognize and ideas worth stopping for. Tags favor specific, searchable terms over generic categories based on real engagement data.
